...
def init_db():
    ...
    cursor.execute("""
        CREATE TABLE IF NOT EXISTS preprompt_usage (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            preprompt_id INTEGER NOT NULL,
            used_at TEXT NOT NULL,
            message_count INTEGER DEFAULT 1,
            FOREIGN KEY (preprompt_id) REFERENCES preprompts(id)
        )
    """)
...
def track_usage(preprompt_name: str) -> Dict:
    """Track preprompt usage."""
    conn = sqlite3.connect(DB_PATH)
    cursor = conn.cursor()
    
    # Get preprompt ID
    cursor.execute("SELECT id FROM preprompts WHERE name = ?", (preprompt_name,))
    row = cursor.fetchone()
    
    if not row:
        conn.close()
        return {"success": False, "error": "Preprompt not found"}
    
    preprompt_id = row[0]
    used_at = datetime.now().isoformat()
    
    cursor.execute("""
        INSERT INTO preprompt_usage (preprompt_id, used_at)
        VALUES (?, ?)
    """, (preprompt_id, used_at))
    
    conn.commit()
    conn.close()
    
    return {"success": True, "preprompt_id": preprompt_id}
...
def get_usage_stats() -> List[Dict]:
    ...
    cursor.execute("""
        ...
        FROM preprompts p
        LEFT JOIN preprompt_usage u ON p.id = u.preprompt_id
        WHERE p.is_active = 1
        GROUP BY p.id
