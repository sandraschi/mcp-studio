        ...
        // Safe JSON parse - avoids "unexpected character at line 1 column 1" when API returns HTML/empty
        async function safeJson(res) {
            const text = await res.text();
            if (!text || !text.trim()) return {};
            const trimmed = text.trim();
            if (trimmed.charAt(0) !== '{' && trimmed.charAt(0) !== '[') {
                console.warn('API returned non-JSON (likely HTML/error):', trimmed.substring(0, 80));
                return {};
            }
            try {
                return JSON.parse(text);
            } catch (e) {
                console.error('JSON parse failed:', e.message, 'body:', trimmed.substring(0, 200));
                return {};
            }
        }
            ...
            modalContent.innerHTML = `
                <div class="space-y-4">
                    ...
                    <div>
                        ...
                        <p class="text-sm text-gray-300">${client.platform} • ${client.client_type}</p>
                    </div>
                    
                    ${client.homepage ? `
                    <div>
                        <h3 class="text-sm font-semibold text-gray-400 mb-2">Homepage</h3>
                        <a href="${client.homepage}" target="_blank" class="text-sm text-blue-400 hover:text-blue-300 underline">🌐 ${client.homepage}</a>
                    </div>
                    ` : ''}
                    ...
                    <div>
                        ...
                        <a href="${client.documentation}" target="_blank" class="text-sm text-blue-400 hover:text-blue-300 underline">📚 Documentation</a>
                    </div>
                    ` : ''}
                    ...
                    <div>
                        ...
                        <a href="${client.github}" target="_blank" class="text-sm text-blue-400 hover:text-blue-300 underline">🐙 GitHub Repository</a>
                    </div>
                    ` : ''}
        ...
        async function showAddServerForm(clientName) {
            ...
            try {
                ...
                availableServers = Array.isArray(parsed) ? parsed : (parsed.servers || parsed.data || []);
            } catch (e) {
            ...
            document.getElementById('add-server-form').addEventListener('submit', async (e) => {
                ...
                try {
                    ...
                    if (res.ok) {
                        ...
                        alert(`Error: ${error.detail || error.message || 'Failed to add server'}`);
                    }
                } catch (e) {
            ...
            try {
                ...
                progressInterval = setInterval(async () => {
                    try {
                        ...
                        const progress = await safeJson(progressRes);
                        ...
                        if (progress && progress.status === 'scanning') {
                        ...
                        } else if (progress && progress.status === 'complete') {
                            ...
                            reposData = Array.isArray(data.data) ? data.data : (data.data || []);
                            renderRepos();
        ...
        async function loadLogs() {
            try {
                ...
                log.innerHTML = logs.map(l => '<div class="text-gray-300">' + l + '</div>').join('') || '<div class="text-gray-500">No activity</div>';
...
Please provide helpful, specific advice about MCP server development, tool design, and best practices.`;
            ...
            const helpContent = `
                <div class="space-y-4">
                    ...
                    <div class="space-y-3">
                        ...
                        <div>
                            <h4 class="font-semibold text-white mb-2">🤖 AI Assistant Tab</h4>
                            <p class="text-sm text-gray-400">Chat with AI about your MCP servers using Ollama.</p>
                        </div>
                        
                        <div class="pt-3 border-t border-white/10">
                            <h4 class="font-semibold text-white mb-2">⌨️ Keyboard Shortcuts</h4>
        ...
        function showLogs() {
            ...
            modalTitle.innerHTML = '📋 SOTA Logger - State of the Art Logging';
            modalContent.innerHTML = `
                <div class="space-y-6">
                    ...
                    <div class="flex items-center gap-4">
                        <div class="flex-1">
                            <input type="text" id="log-search" placeholder="🔍 Search logs..." class="w-full px-3 py-2 bg-white/5 border border-white/10 rounded text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" onkeyup="filterLogs()">
                        </div>
                    ...
                    <div class="bg-black/20 rounded-lg border border-white/10">
                        <div class="px-4 py-3 border-b border-white/10 flex items-center justify-between">
                            <h4 class="font-semibold text-sm">📊 Log Stream</h4>
...
▶️ SOTA Logger - Advanced logging with real-time monitoring, intelligent filtering, and performance analytics
                    </div>
                </div>
            `;
        ...
        function clearLogs() {
            if (confirm('🗑️ Clear all logs? This action cannot be undone.')) {
                fetch('/api/logs/clear', { method: 'POST' })
                    .then(() => {
                        showToast('🗑️ Logs cleared', 'success');
                        loadLoggerData();
                    })
                    .catch(e => showToast('❌ Failed to clear logs: ' + e.message, 'error'));
            }
        }
